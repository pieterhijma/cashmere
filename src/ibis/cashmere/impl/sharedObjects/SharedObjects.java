...
public final class SharedObjects implements Config {
    ...
    public SharedObjects(Cashmere s) {
        this.s = s;
    ...
    public void addObject(SharedObject object) {
        if (! SO_ENABLED) {
            throw new Error("Shared objects not enabled! Did you set cashmere.so to false?");
        }
        ...
        soLogger.debug("CASHMERE '" + s.ident + "': " + "object added, id = "
            + object.getObjectId());
    }
    ...
    private void handleSOInvocations() {
        while (true) {
            ...
            try {
                ...
                // No need to hold the cashmere lock here.
    ...
    private void doExecuteGuard(InvocationRecord r)
        ...
        if (objRefs == null || objRefs.isEmpty()) {
            soLogger.error("CASHMERE '" + s.ident + "': "
                ...
                + "have shared objects.\n"
                + "This is not a correct Cashmere program.");
            System.exit(1);
        }
