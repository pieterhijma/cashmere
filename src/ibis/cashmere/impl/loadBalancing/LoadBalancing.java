...
public final class LoadBalancing implements Config {
    ...
    final class StealRequestHandler extends Thread {
        ...
        public StealRequestHandler() {
            setDaemon(true);
            setName("Cashmere StealRequestHandler");
        }
    ...
    public LoadBalancing(Cashmere s) {
        this.s = s;

	this.timer = MCCashmere.createTimer("java", "loadbalance", "steal",
		1000);
    ...
    public InvocationRecord stealJob(Victim v, boolean blockOnServer) {
        if (ASSERTS) {
            synchronized(this) {
        	if (stolenJob != null) {
        	    ftLogger.error("CASHMERE '" + s.ident + "': EEK: stealing while stolenJob is non-null!");
                    throw new Error(
        ...
        if (Cashmere.GLOBAL_PAUSE_RESUME) {
            synchronized (s) {
                if (s.comm.paused) {
                    ...
                    commLogger.info("CASHMERE '" + s.ident + "': paused for "
                        + (end - start) + " ms");
                }
            }
        }
    ...
    public void queueStealRequest(SendPortIdentifier ident, int opcode) {
        ...
        synchronized (stealQueue) {
            if (stealLogger.isInfoEnabled()) {
                stealLogger.info("Queueing steal request from " + ident);
            }
            stealQueue.add(r);
