/* $Id: GlobalResultTable.java 11556 2009-11-21 10:56:33Z ceriel $ */
...
public final class GlobalResultTable implements Config, Protocol {
    ...
    protected GlobalResultTable(Cashmere s) {
        this.s = s;
        ...
        try {
            ...
            if (value != null) {
                if (grtLogger.isDebugEnabled()) {
                    grtLogger
                    .debug("CASHMERE '" + s.ident + "': lookup successful " + key);
                }
                ...
                if (grtLogger.isDebugEnabled()) {
                    grtLogger
                    .debug("CASHMERE '" + s.ident + "': lookup failed " + key);
                }
            }
    ...
    private void update(Stamp key, GlobalResultTableValue value) {
        ...
        try {
            ...
            grtLogger.debug("CASHMERE '" + s.ident + "': update complete: " + key
                + "," + value);
        ...
        try {
            ...
            if (grtLogger.isDebugEnabled()) {
                for (Stamp key : updates.keySet()) {
                    grtLogger.debug("CASHMERE '" + s.ident + "': update: " + key
                            + "," + updates.get(key));
                }
            }
        } finally {
        ...
        try {
            for(int i=0; i<size; i++) {
                ...
                try {
                    ...
                    grtLogger.debug("CASHMERE '" + s.ident + "': " + msgSize
                            + " sent in "
                            + s.stats.tableSerializationTimer.lastTimeVal() + " to "
                            + v);
                } catch (IOException e) {
                    m.finish(e);
                    grtLogger.info("CASHMERE '" + s.ident + "': Got exception in finish()");
                    //always happens after a crash
                }
            }
