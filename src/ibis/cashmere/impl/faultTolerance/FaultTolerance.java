...
import ibis.cashmere.impl.checkPointing.CheckpointAndQuitThread;
import ibis.cashmere.impl.checkPointing.CheckpointFile;
import ibis.cashmere.impl.checkPointing.CheckpointThread;
...
import ibis.cashmere.impl.spawnSync.StampVector;
...
public final class FaultTolerance implements Config {
     ...
     *  table; protected by the Cashmere lock. */
    ...
    public FaultTolerance(Cashmere s) {
        this.s = s;
    ...
    public void handleCrashes() {
        ftLogger.debug("CASHMERE '" + s.ident + ": handling crashes");
        ...
        try {
            ...
            synchronized (s) {
                while (crashesToHandle.size() > 0) {
                    ...
                    ftLogger.debug("CASHMERE '" + s.ident + ": handling crash of "
                        + id);
    ...
    public void addToAbortAndStoreList(Stamp stamp) {
        Cashmere.assertLocked(s);
        abortLogger.debug("CASHMERE '" + s.ident + ": got abort message");
    ...
    public void deleteCluster(String cluster) {
        ftLogger.info("CASHMERE '" + s.ident + "': delete cluster " + cluster);
    ...
    void handleCheckpointAndQuit() {
        synchronized(s) {
            if (coordinator) {
                takeCoordinatorCheckpoint();
                System.out.println("CASHMERE '" + s.ident
                                   + "': checkpoint taken");
