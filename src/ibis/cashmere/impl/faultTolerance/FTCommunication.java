...
import ibis.cashmere.impl.checkPointing.Checkpoint;
...
final class FTCommunication implements Config, ReceivePortConnectUpcall,
    ...
    protected FTCommunication(Cashmere s) {
        this.s = s;
    }

    protected void electClusterCoordinator() {
        try {
            String election = "cashmere " + Victim.clusterOf(s.comm.ibis.identifier())
                        + " cluster coordinator";
            if (s.ft.clusterCoordinatorIdent == null) {
                // Note: if the cashmere.masterHost property is set, the code below only
            ...
            ftLogger.error("CASHMERE '" + s.ident + "': Could not start ibis: "
                    + e, e);
    ...
    protected boolean askForJobResult(InvocationRecord r) {
        ...
        if (value.type == GlobalResultTableValue.TYPE_POINTER) {
            ...
            synchronized (s) {
                ...
                grtLogger.debug("CASHMERE '" + s.ident
            ...
            try {
                ...
                grtLogger.warn("CASHMERE '" + s.ident
    ...
    protected void sendAbortAndStoreMessage(InvocationRecord r) {
        ...
        abortLogger.debug("CASHMERE '" + s.ident
    ...
    public void lostConnection(ReceivePort me, SendPortIdentifier johnDoe,
        ...
        if (reason == null) {
        	...
        	ftLogger.info("CASHMERE '" + s.ident + "': got lostConnection upcall: "
    ...
    public void joined(IbisIdentifier joiner) {
        ftLogger.debug("CASHMERE '" + s.ident + "': got join of " + joiner);
    ...
    protected void handleJoins(IbisIdentifier[] joiners) {
        ftLogger.debug("CASHMERE '" + s.ident + "': dealing with "
                + joiners.length + " joins");

        s.so.handleJoins(joiners);
        ftLogger.debug("CASHMERE '" + s.ident + "': SO ports created");

        for (int i = 0; i < joiners.length; i++) {
            IbisIdentifier joiner = joiners[i];

            ftLogger.debug("CASHMERE '" + s.ident + "': creating sendport");
            ...
            try {
                ...
                ftLogger.warn("CASHMERE '" + s.ident
                        + "': got an exception in Cashmere.join", e);
                continue;
            }
            ftLogger.debug("CASHMERE '" + s.ident + "': creating sendport done");
            ...
            ftLogger.debug("CASHMERE '" + s.ident + "': " + joiner + " JOINED");
        }
    }
        ...
        try {
            ...
            synchronized (s) {
                ...
                if (ASSERTS && value == null) {
                    ...
                    grtLogger.error("CASHMERE '" + s.ident + "': EEK!!! " + ident
    ...
    protected void handleResultPush(ReadMessage m) {
        grtLogger.info("CASHMERE '" + s.ident + ": handle result push");
        try {
        ...
        grtLogger.info("CASHMERE '" + s.ident + ": handle result push finished");
    }
    ...
    protected void pushResults(Victim victim,
        ...
        try {
            ...
            if (grtLogger.isDebugEnabled()) {
                grtLogger.debug("CASHMERE '" + s.ident + "': " + numBytes
                        + " bytes pushed");
            }
        } catch (IOException e) {
            if (m != null) {
            ...
            if (e instanceof NotSerializableException || e instanceof InvalidClassException) {
                ...
                grtLogger.info("CASHMERE '" + s.ident + "': error pushing results "
                        + e);
            }
        }
    }
