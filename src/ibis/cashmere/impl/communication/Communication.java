 ...
 */
package ibis.cashmere.impl.communication;
...
import ibis.cashmere.impl.Statistics;
...
public final class Communication implements Config, Protocol {
    ...
    public Communication(Cashmere s) {
        this.s = s;

        IbisCapabilities ibisProperties = createIbisCapabilities();

        commLogger.debug("CASHMERE '" + "- " + "': init ibis");

        portType = createCashmerePortType();
        ...
        commLogger.debug("CASHMERE '" + "- " + "': init ibis DONE, "
                + "my cluster is '" + Victim.clusterOf(ident) + "'");

        try {
            MessageHandler messageHandler = new MessageHandler(s);

            receivePort = ibis.createReceivePort(portType, "cashmere port",
        ...
        if (CLOSED) {
            ...
            commLogger.info("CASHMERE '" + ident + "': running with open world");
        }
    }

    public IbisIdentifier electMaster() {
        String election = "cashmere.master";
        ...
        while (winner == null) {
            if (MASTER_HOST != null) {
                try {
                    ...
                    commLogger.warn("cashmere.masterhost is set to an unknown "
                            + "name: " + MASTER_HOST);
    ...
    public PortType createCashmerePortType() {
    ...
    public void bcastMessage(byte opcode) {
        ...
        for (int i = 0; i < victims.length; i++) {
            ...
            Victim v = victims[i];
            commLogger.debug("CASHMERE '" + s.ident + "': sending "
            ...
            try {
                ...
                synchronized (s) {
                    ...
                    try {
                        ...
                        ftLogger.warn("CASHMERE '" + s.ident
    ...
    public static ReceivePortIdentifier connect(SendPort s,
        ...
        connLogger.info("CASHMERE '" + id + "': connecting to " + name + " at "
                + ident);
        while (r == null && timeLeft > 0) {
            try {
                r = s.connect(ident, name, timeLeft, false);
            } catch (AlreadyConnectedException x) {
                connLogger.info("CASHMERE '" + id + "': already connected to "
                        + name + " at " + ident, x);
                ...
                connLogger
                        .info("CASHMERE '" + id + "': IOException in connect to "
                                + ident + ": " + e, e);
                try {
                    Thread.sleep(500);
                } catch (InterruptedException e2) {
    ...
    private void barrier() {
        IbisIdentifier ident = ibis.identifier();
        commLogger.debug("CASHMERE '" + ident + "': barrier start");
        ...
        try {
            ...
            commLogger.warn("CASHMERE '" + ident + "': error in barrier", e);
        }

        commLogger.debug("CASHMERE '" + ident + "': barrier DONE");
    }
    ...
    public void sendExitAck() {
        ...
        try {
            commLogger.debug("CASHMERE '" + s.ident
		...
		writeMessage.writeObject(s.mcStats);
		writeMessage.writeObject(s.timeSyncInfo);
            }
            v.finish(writeMessage);
        } catch (IOException e) {
            if (writeMessage != null) {
            ...
            ftLogger.info(
                    "CASHMERE '" + s.ident + "': could not send exit message to "
                            + s.getMasterIdent(), e);
            try {
    ...
    public void waitForAllNodes() {
        commLogger.debug("CASHMERE '" + s.ident + "': pre barrier");
        ...
        commLogger.debug("CASHMERE '" + s.ident + "': post barrier");
    }
    ...
    public void handleExitReply(ReadMessage m) {
        ...
        commLogger.debug("CASHMERE '" + s.ident + "': got exit ACK message from "
                + ident.ibisIdentifier());

        if (STATS) {
            try {
                Statistics stats = (Statistics) m.readObject();
		MCStats mcStats = (MCStats) m.readObject();
		TimeSyncInfo timeSyncInfo = (TimeSyncInfo) m.readObject();
                s.totalStats.add(stats);
		s.totalMCStats.add(mcStats);
		s.timeSyncInfo.putAll(timeSyncInfo);
            } catch (Exception e) {
                commLogger.warn("CASHMERE '" + s.ident
    ...
    public void handleExitMessage(IbisIdentifier ident) {
        commLogger.debug("CASHMERE '" + s.ident + "': got exit message from "
                + ident);
    ...
    public void handleExitStageTwoMessage(IbisIdentifier ident) {
        commLogger.debug("CASHMERE '" + s.ident + "': got exit2 message from "
                + ident);
    ...
    public void pause() {
        ...
        soBcastLogger.info("CASHMERE '" + s.ident + "': sending pause");
        Victim[] victims;
        synchronized (s) {
            victims = s.victims.victims();
    ...
    public void resume() {
        soBcastLogger.info("CASHMERE '" + s.ident + "': sending resume");
        ...
        for (int i = 0; i < victims.length; i++) {
            try {
                ...
                commLogger.info("CASHMERE '" + s.ident
                        + "': could not send pause message: " + e);
                // ignore
            }
    ...
    void gotPause() {
        soBcastLogger.debug("CASHMERE '" + s.ident + "': got pause");
    ...
    void gotResume() {
        soBcastLogger.debug("CASHMERE '" + s.ident + "': got resume");
    ...
    // this should be called by the master
    public void initializeManyCore(Map<String, List<String>> defines, int cnt, int sz) {
	Victim[] victims;
	synchronized (s) {
	    victims = s.victims.victims();
	}
	for (int i = 0; i < victims.length; i++) {
	    try {
		WriteMessage m = victims[i].newMessage();
                m.writeByte(MANY_CORE_INIT);
		m.writeObject(defines);
                m.writeInt(cnt);
                m.writeInt(sz);
		victims[i].finish(m);
	    }
		...
		commLogger.info("CASHMERE '" + s.ident + 
			"': could not send initManyCore message: " + e);
	    }
	}
	MCCashmere.initialize2(defines, cnt, sz);
	barrier();
    }


    void gotManyCoreInit(ReadMessage m) throws IOException, ClassNotFoundException {
        @SuppressWarnings("unchecked")
        Map<String, List<String>> defines = 
                (Map<String, List<String>>) m.readObject();
        int cnt = m.readInt();
        int sz = m.readInt();
        m.finish();
        MCCashmere.initialize2(defines, cnt, sz);
