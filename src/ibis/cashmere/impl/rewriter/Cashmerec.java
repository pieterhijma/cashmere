/* $Id: Cashmerec.java 14659 2012-04-20 21:07:23Z ceriel $ */

package ibis.cashmere.impl.rewriter;
...
public final class Cashmerec extends IbiscComponent {
    private JavaClass cashmereObjectClass;
    ...
    private ObjectType cashmereType;
    ...
    // for ibisc
    private HashSet<String> cashmereSet = new HashSet<String>();
    ...
    public Cashmerec(boolean verbose, boolean local, boolean keep,
    ...
    Instruction getCashmere(InstructionFactory insf) {
        return insf.createInvoke("ibis.cashmere.impl.Cashmere", "getCashmere",
            cashmereType, Type.NO_ARGS, Constants.INVOKESTATIC);
    }
    ...
    String invocationRecordName(Method m, String clnam, String pnam) {
        if (pnam != null && !pnam.equals("")) {
            return pnam + ".Cashmere_" + clnam + "_" + do_mangle(m)
                + "_InvocationRecord";
        }
        return "Cashmere_" + clnam + "_" + do_mangle(m) + "_InvocationRecord";
    }

    String invocationRecordFileBase(Method m, String clnam, String pnam) {
        return getFileBase(pnam, clnam + "_" + do_mangle(m), "Cashmere_",
            "_InvocationRecord");
    }
    ...
    String localRecordName(Method m) {
        if (packageName != null && !packageName.equals("")) {
            return packageName + ".Cashmere_" + classNameNoPackage + "_"
                + do_mangle(m) + "_LocalRecord";
        }
        return "Cashmere_" + classNameNoPackage + "_" + do_mangle(m)
            + "_LocalRecord";
    }

    String localRecordFileBase(Method m) {
        return getFileBase(packageName,
            classNameNoPackage + "_" + do_mangle(m), "Cashmere_", "_LocalRecord");
    }
    ...
    String returnRecordName(Method m, String clnam, String pnam) {
        ...
        return "Cashmere_" + clnam + "_" + do_mangle(m) + "_ReturnRecord";
    }
    ...
    void rewriteSync(MethodGen m, InstructionList il, InstructionHandle i,
        ...
        Instruction sync_invocation = ins_f.createInvoke(
            "ibis.cashmere.impl.Cashmere", "sync", Type.VOID,
            new Type[] { spawnCounterType }, Constants.INVOKEVIRTUAL);
        Instruction cashmere_field_access = getCashmere(ins_f);
        ...
        i.getPrev().setInstruction(cashmere_field_access);
        i.setInstruction(new ALOAD(maxLocals));

        i = i.getNext(); // so that we can insert ...

        // and call Cashmere.sync 
        ...
        il.insert(pos, ins_f.createFieldAccess(
            "ibis.cashmere.impl.spawnSync.InvocationRecord", "cacheNext", irType,
            Constants.GETFIELD));
        ...
        for (int k = 0; k < idTable.size(); k++) {
            ...
            if (idTable.size() > 1) {
                ...
                il.insert(pos, ins_f.createFieldAccess(
                    "ibis.cashmere.impl.spawnSync.InvocationRecord", "storeId",
    ...
    void rewriteSpawn(MethodGen m, InstructionList il, Method target,
        ...
        // Finally we call Cashmere.spawn(outstandingSpawns) 
        ...
        parameters[ix++] = Type.INT;
        parameters[ix++] = Type.INT;
        parameters[ix++] = new ObjectType("ibis.cashmere.impl.aborts.LocalRecord");
        ...
        // Now, we call Cashmere.spawn(outstandingSpawns) 

        // push s 
        ih = il.append(ih, getCashmere(ins_f));

        // push outstandingSpawns 
        ih = il.append(ih, new ALOAD(maxLocals + 1));

        // and call Cashmere.spawn 
        ih = il.append(ih, ins_f.createInvoke("ibis.cashmere.impl.Cashmere", "spawn",
    ...
    void rewriteMethod(Method mOrig, MethodGen m) {
        ...
        il.insert(insertAllocPos, ins_f.createInvoke(
            "ibis.cashmere.impl.spawnSync.SpawnCounter", "newSpawnCounter",
        ...
        m.addLocalVariable("local_record", new ObjectType(
            "ibis.cashmere.impl.aborts.LocalRecord"), maxLocals + 3, lr_h, null);
            ...
            if (!mtab.isClone(m) && mtab.containsInlet(m)) {
                ...
                Method exceptionHandler = recgen.containsMethod(
                    "handleException", "(ILjava/lang/Throwable;"
                        + "Libis/cashmere/impl/spawnSync/InvocationRecord;)V");
    ...
    void writeLocalRecord(Method m) throws IOException {
        ...
        try {
            ...
            out.println("public final class " + name
                + " extends ibis.cashmere.impl.aborts.LocalRecord {");
            ...
            out
                .println("    public void handleException(int spawnId, "
                    + "Throwable t, ibis.cashmere.impl.spawnSync.InvocationRecord parent) "
                    + "throws Throwable {");
            out
                .println("        if (ibis.cashmere.impl.Config.inletLogger.isDebugEnabled()) {");
            out.println("            ibis.cashmere.impl.Config.inletLogger.debug("
        ...
        try {
            ...
            for (int i = 0; i < params.length; i++) {
                if (params[i] instanceof ObjectType) {
                    String clnam = ((ObjectType) params[i]).getClassName();
                    if (implementationOf(clnam,
                        "ibis.cashmere.many_core.ZeroCopy")) {
                	hasZeroCopy = true;
                    }
                }
            }
            ...
            out.println("public final class " + name
                + " extends ibis.cashmere.impl.spawnSync.InvocationRecord {");
            ...
            for (int i = 0; i < params_types_as_names.length; i++) {
                if (!isSharedObject(params[i])) {
                    ...
                    /*out.println("    transient " + params_types_as_names[i]
                     + " param" + i + ";");*/
                    out.println("    public String cashmere_so_reference_param"
                        + i + ";");
                }
            }
            ...
            out
                .println("ibis.cashmere.impl.spawnSync.SpawnCounter s, "
                    + "ibis.cashmere.impl.spawnSync.InvocationRecord next, int storeId, "
                    + "int spawnId, ibis.cashmere.impl.aborts.LocalRecord parentLocals) {");
            ...
            for (int i = 0; i < params_types_as_names.length; i++) {
                if (!isSharedObject(params[i])) {
                    ...
                    out.println("        this.cashmere_so_reference_param" + i
            ...
            if (invocationRecordCache) {
                ...
                for (int i = 0; i < params_types_as_names.length; i++) {
                    if (!isSharedObject(params[i])) {
                        ...
                        out.println("        res.cashmere_so_reference_param" + i
                            + " = param" + i + ".getObjectIdAndSetNonlocal();");
                    }
                }
            ...
            if (!returnType.equals(Type.VOID)) {
                ...
                out.println("ibis.cashmere.impl.spawnSync.SpawnCounter s, "
                    + "ibis.cashmere.impl.spawnSync.InvocationRecord next, "
                    + "int storeId, int spawnId, "
                    + "ibis.cashmere.impl.aborts.LocalRecord parentLocals) {");
            ...
            for (int i = 0; i < params.length; i++) {
                if (params[i] instanceof ReferenceType) {
                    if (!isSharedObject(params[i])) {
                        ...
                        out.println("        w.cashmere_so_reference_param" + i
                            + " = null;");
                    }
                }
            }
                ...
                if (isSharedObject(params[i])) {
                    out
                        .print("("
                            ...
                            + ")"
                            + "ibis.cashmere.impl.Cashmere.getCashmere().so.getSOReference(cashmere_so_reference_param"
                            + i + ")");
                } else {
                    out.print("param" + i);
                }
            ...
            out
                .println("            } catch (Throwable e) {\n"
                    + "                if (ibis.cashmere.impl.Config.inletLogger.isDebugEnabled()) {\n"
                    + "                    ibis.cashmere.impl.Config.inletLogger.debug("
                    + "\"caught exception in runlocal: "
                    + "\" + e, e);\n"
                    + "                }\n"
                    + "                if (! (e instanceof ibis.cashmere.impl.aborts.AbortException)) {\n"
                    + "                    eek = e;\n"
                    + "                } else if (ibis.cashmere.impl.Config.abortLogger.isDebugEnabled()) {\n"
                    + "                    ibis.cashmere.impl.Config.abortLogger.debug("
                    ...
                    + "                if (ibis.cashmere.impl.Config.inletLogger.isDebugEnabled()) {\n"
                    + "                    ibis.cashmere.impl.Config.inletLogger.debug(\"runlocal:"
            ...
            // runRemote method 
            out.println("    public ibis.cashmere.impl.spawnSync.ReturnRecord "
                + "runRemote() {");
            out.println("        try {");
            out.print("            ");
            if (!returnType.equals(Type.VOID)) {
            ...
            out.println("        } catch (Throwable e) {\n"
                      ...
                      + "            }\n");
            out
                .println("                if (! (e instanceof ibis.cashmere.impl.aborts.AbortException)) {");
            out.println("                    eek = e;");
            out
                .println("                } else if (ibis.cashmere.impl.Config.abortLogger.isDebugEnabled()) {");
            out
                .println("                    ibis.cashmere.impl.Config.abortLogger.debug("
                    + "\"got AbortException \" + e, e);");
            out.println("                }");
            out.println("        }");
            out
            ...
            if (returnType.equals(Type.VOID)) {
                out
                    .println("    public ibis.cashmere.impl.spawnSync.ReturnRecord "
                        + "getReturnRecord() {");
                out
                    .println("        return new "
                        + returnRecordName(m, clname, pnam)
            ...
            out
                .println("        throws ibis.cashmere.impl.sharedObjects.SOReferenceSourceCrashedException {");
            for (int i = 0; i < params.length; i++) {
                if (isSharedObject(params[i])) {
                    /*		    out.println("        param" + i + " = "
                     + "(" + params_types_as_names[i] + ")"
                     + "cashmere.getSOReference(cashmere_so_reference_param"
                     + i + ", owner);");*/
                    out
                        .println("        ibis.cashmere.impl.Cashmere.getCashmere().so.setSOReference(cashmere_so_reference_param"
                            + i + ", owner);");
                }
            }
            out.println("    }\n");
            ...
            for (int i = 0; i < params.length; i++) {
                if (isSharedObject(params[i])) {
                    out.println("        refs.add(cashmere_so_reference_param" + i
                        + ");");
                }
            }
            ...
            if (hasZeroCopy) {
        	out.println("    public java.util.List<java.nio.ByteBuffer> getByteBuffers() {");
        	out.println("        java.util.List<java.nio.ByteBuffer> l = new java.util.ArrayList<java.nio.ByteBuffer>();");
        	for (int i = 0; i < params.length; i++) {
        	    if (params[i] instanceof ObjectType) {
        		String clnam = ((ObjectType) params[i]).getClassName();
        		if (implementationOf(clnam,
        			"ibis.cashmere.many_core.ZeroCopy")) {
        		    out.println("        param" + i + ".getByteBuffers(l);");
        		}
        	    }
        	}
        	...
        	out.println("    public void setByteBuffers(java.util.List<java.nio.ByteBuffer> l) {");
        	for (int i = 0; i < params.length; i++) {
        	    if (params[i] instanceof ObjectType) {
        		String clnam = ((ObjectType) params[i]).getClassName();
        		if (implementationOf(clnam,
        			"ibis.cashmere.many_core.ZeroCopy")) {
        		    out.println("        param" + i + ".setByteBuffers(l);");
        		}
        	    }
        	}
        	out.println("    }");
    ...
    void writeReturnRecord(Method m, String clname, String pnam)
        ...
        try {
            ...
            out.println("public final class " + name
                + " extends ibis.cashmere.impl.spawnSync.ReturnRecord {");
            if (!returnType.equals(Type.VOID)) {
                out.println("    " + returnType + " result;\n");
            }
            ...
            if (!returnType.equals(Type.VOID)) {
                out
                    .println(returnType
                        + " result, Throwable eek, ibis.cashmere.impl.spawnSync.Stamp stamp) {");
            } else {
                out
                    .println(" Throwable eek, ibis.cashmere.impl.spawnSync.Stamp stamp) {");
            }
            ...
            out
                .println("    public void assignTo(ibis.cashmere.impl.spawnSync.InvocationRecord rin) {");
           ...
           if (returnType instanceof ObjectType) {
               String clnam = ((ObjectType) returnType).getClassName();
               if (implementationOf(clnam,
        	       "ibis.cashmere.many_core.ZeroCopy")) {
        	   out.println("    public java.util.List<java.nio.ByteBuffer> getByteBuffers() {");
        	   out.println("        java.util.List<java.nio.ByteBuffer> l = new java.util.ArrayList<java.nio.ByteBuffer>();");

        	   out.println("        result.getByteBuffers(l);");

        	   out.println("        return l;");
        	   out.println("    }");
        	   
        	   out.println("    public void setByteBuffers(java.util.List<java.nio.ByteBuffer> l) {");
        	   out.println("        result.setByteBuffers(l);");
        	   out.println("    }");
        	}
            }
    ...
    String SOInvocationRecordName(Method m, String clnam, String pnam) {
        ...
        return "Cashmere_" + clnam + "_" + do_mangle(m) + "_SOInvocationRecord";
    }
    ...
    private void writeSOInvocationRecord(Method m, String clname, String pnam)
        ...
        out.println("public final class " + name
            + " extends ibis.cashmere.impl.sharedObjects.SOInvocationRecord {");
        ...
        //invoke method
        out.println("\tpublic void invoke(ibis.cashmere.SharedObject object) {");
    ...
    void rewriteSOMethod(Method m, String clnam, String pnam) {
        ...
        //Cashmere.getCashmere().
        //broadcastSOInvocation(new Cashmere_methodname_SOInvocationRecord(params))
        //synchronized(Cashmere.getCashmere()) {
        //return so_local_methodname(params);
        //}

        newMethodInsList = new InstructionList();
        //broadcast 
        newMethodInsList.append(ins_f.createInvoke("ibis.cashmere.impl.Cashmere",
            "getCashmere", new ObjectType("ibis.cashmere.impl.Cashmere"), new Type[] {},
            Constants.INVOKESTATIC));
        ...
        newMethodInsList.append(ins_f.createInvoke("ibis.cashmere.SharedObject",
        ...
        newMethodInsList.append(ins_f.createInvoke("ibis.cashmere.impl.Cashmere",
            "broadcastSOInvocation", Type.VOID, new Type[] { new ObjectType(
                "ibis.cashmere.impl.sharedObjects.SOInvocationRecord") },
            Constants.INVOKEVIRTUAL));
        //enter the monitor	
        monitorVarAddr = 1;
        for (int k = 0; k < arguments.length; k++) {
    ...
    void doSORewrite() throws IOException {
        ...
        /*		int getCashmere = cpg.addMethodref("ibis.cashmere.impl.Cashmere",
         "getCashmere",
         "()Libis/cashmere/impl/Cashmere");
         int broadcastSOInvocation = 
         cpg.addMethodref("ibis.cashmere.impl.Cashmere",
         "broadcastSOInvocation",
         "(Libis/cashmere/impl/SOInvocationRecord)V");*/
        ...
        for (int j = 0; j < methods.length; j++) {
            ...
            if (a.isSpecial(method)) {
                ...
                //Cashmere.getCashmere().broadcastSOInvocation(new SOInvRecord())
    ...
    public void start(JavaClass clazz) {
        ...
        if (isCashmere()) {
            if (verbose) {
                System.out.println(className + " is a cashmere class");
            }
        }
        ...
        try {
            if (Repository.instanceOf(c, "ibis.cashmere.SharedObject")) {
        ...
        if (main != null) {
            ...
            // FieldGen f = new FieldGen(Constants.ACC_STATIC, cashmereType,
            //         cashmereFieldName, gen_c.getConstantPool());

            // cashmereField = f.getField();
            // gen_c.addField(cashmereField);
    ...
    public static void usage() {
        System.err.println("Usage : java Cashmerec [[-no]-verbose] [[-no]-keep] "
            + "[-dir|-local] [[-no]-irc] [[-no]-sc-opt] "
            + "[[-no]-inlet-opt] [[-no]-so] " + "<classname>*");
        System.exit(1);
    }

    private void addToCashmereSet(String list) {
        StringTokenizer st = new StringTokenizer(list, ", ");
        while (st.hasMoreTokens()) {
            cashmereSet.add(st.nextToken());
        }
    }
    ...
    public boolean processArgs(ArrayList<String> args) {
        ...
        for (int i = 0; i < args.size(); i++) {
            ...
            if (false) {
                // nothing
            } else if (arg.equals("-cashmere-no-irc")) {
                invocationRecordCache = false;
                args.remove(i--);
            } else if (arg.equals("-cashmere-irc")) {
                invocationRecordCache = true;
                args.remove(i--);
            } else if (arg.equals("-cashmere-no-inlet-opt")) {
                inletOpt = false;
                args.remove(i--);
            } else if (arg.equals("-cashmere-inlet-opt")) {
                inletOpt = true;
                args.remove(i--);
            } else if (arg.equals("-cashmere-no-sc-opt")) {
                spawnCounterOpt = false;
                args.remove(i--);
            } else if (arg.equals("-cashmere-sc-opt")) {
                spawnCounterOpt = true;
                args.remove(i--);
            } else if (arg.equals("-cashmere")) {
                args.remove(i);
                retval = true;
                if (i >= args.size()) {
                    throw new IllegalArgumentException("-cashmere needs classlist");
                }
                addToCashmereSet(args.get(i));
                args.remove(i--);
            }
        }
        return retval;
    ...
    public void process(Iterator<?> classes) {
        if (cashmereSet.size() == 0) {
            return;
        }
        while (classes.hasNext()) {
            JavaClass clazz = (JavaClass) classes.next();
            if (cashmereSet.contains(clazz.getClassName())) {
                start(clazz);
            }
        }
    }
    ...
    public static void main(String[] args) {
        ...
        Cashmerec cashmerec = new Cashmerec(verbose, local, keep, invocationRecordCache,
                inletOpt, spawnCounterOpt);

        for (int i = 0; i < list.size(); i++) {
            cashmerec.start(lookup(list.get(i)));
        }
    }
}
