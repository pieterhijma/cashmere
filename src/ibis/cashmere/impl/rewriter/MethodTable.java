/* $Id: MethodTable.java 14659 2012-04-20 21:07:23Z ceriel $ */
...
final class MethodTable {
    ...
    static class LocalEntry {
        ...
        public boolean equals(Object o) {
            if (! (o instanceof LocalEntry)) {
                return false;
            }
            LocalEntry e = (LocalEntry) o;
            return e.offset == offset && e.tp == tp;
        }
    }
	...
	LocalVariableGen[] locals = m.getLocalVariables();
        ...
        // System.out.println("rewriteLocals: " + m.getName() + ", locals.length = " + locals.length);
        // BCEL problem: if there is a LocalVariableTable AND a LocalVariableTypeTable in the method,
        // the MethodGen only contains one of those.
	...
	for (int i = 0; i < locals.length; i++) {
	    int ind = locals[i].getIndex();
            // System.out.println("Local variable: " + locals[i].getName() + ", index = " + ind);
	    if (ind < parameterpos) {
		continue;
	    }
	...
	// rewritten!
        HashMap<LocalEntry, Integer> newLocals = new HashMap<LocalEntry, Integer>();
	...
	for (InstructionHandle h = il.getStart(); h != null; h = h.getNext()) {
		...
		if (pos >= parameterpos) {
			...
			int local = lins.getIndex();
                        // System.out.println("Found local variable not in table: " + local);
			Type tp = lins.getType(cpg);
                        LocalEntry e = new LocalEntry(local, tp);
			if (newLocals.get(e) == null) {
                            // System.out.println("it is new");
			    newLocals.put(e, Integer.valueOf(maxpos));
			    maxpos++;
			    if (tp.equals(Type.LONG) || tp.equals(Type.DOUBLE)) {
                                // System.out.println("it is a long or double");
				maxpos++;
			    }
			}
			Integer i = newLocals.get(e);
                        // System.out.println("Rewriting to " + i.intValue());
    ...
    MethodTable(JavaClass c, ClassGen gen_c, Cashmerec self, boolean verbose) {
	this.verbose = verbose;
	this.self = self;
	this.c = c;

	try {
	    spawnableClass = Repository.lookupClass("ibis.cashmere.Spawnable");
	} catch (ClassNotFoundException e) {
	    throw new Error("Could not find class ibis.cashmere.Spawnable", e);
	}
	...
	for (int i = 0; i < methods.length; i++) {
	    ...
	    Cashmerec.removeLocalTypeTables(mg);
	    m = mg.getMethod();
	    gen_c.addMethod(m);
