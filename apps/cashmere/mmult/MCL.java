...
class MCL {
    ...
    static void launchMatmulKernel(KernelLaunch kl, int n, int m, int p, 
            ByteBuffer c, ByteBuffer a, ByteBuffer b) throws MCCashmereNotAvailable {
        launchMatmulKernel(kl, n, m, p, c, true, a, true, b, true);
    }


    static void launchMatmulKernel(KernelLaunch kl, int n, int m, int p, 
            ByteBuffer c, boolean copyc, ByteBuffer a, boolean copya, ByteBuffer b, 
        ...
        if (kl.getDeviceName().equals("xeon_phi")) {
            ...
            kl.launch(4 * (m / 4), 1 * (n / 128), 1 * 1, 4, 1, 1);
        }
        else {
