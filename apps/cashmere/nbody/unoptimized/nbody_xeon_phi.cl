...
__kernel void computeAccelerationKernel (int start, int nCompute, int nBodies, 
    ...
    const int body = 16 * tbody + vbody;
    const int bodyOffset = 16 * tbody + vbody + start;
    if (16 * tbody + vbody + start < nBodies) {
    
        float acc0 = 0.0;
        float acc1 = 0.0;
        float acc2 = 0.0;
        const float pos0 = positions[3 * (16 * tbody + vbody + start)];
        const float pos1 = positions[3 * (16 * tbody + vbody + start) + 1];
        const float pos2 = positions[3 * (16 * tbody + vbody + start) + 2];
        ...
        accel[3 * (16 * tbody + vbody)] = acc0;
        accel[3 * (16 * tbody + vbody) + 1] = acc1;
        accel[3 * (16 * tbody + vbody) + 2] = acc2;
    }
}
