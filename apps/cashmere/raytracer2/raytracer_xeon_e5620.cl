...
__kernel void radianceKernel (int h, int w, __global Vec* colors, int height, 
    ...
    const int realX = 4 * tx + vx + xOffset;
    if (y < h) {
    
        uint seed0 = seeds[(y + yOffset) * (2 * width) + 2 * (4 * tx + vx + 
                xOffset)];
        uint seed1 = seeds[(y + yOffset) * (2 * width) + 2 * (4 * tx + vx + 
                xOffset) + 1];
        Ray ray;
        generateCameraRay(camera, &seed0, &seed1, width, height, 4 * tx + vx + 
                xOffset, y + yOffset, &ray);
        Vec r;
        radiancePathTracing(nrSpheres, spheres, &ray, &seed0, &seed1, &r);
        if (currentSample == 0) {
        
            vassign2m(&colors[4 * tx + vx + y * w], &r);
        } else {
        
            const float k1 = currentSample;
            const float k2 = 1.0 / (currentSample + 1.0);
            colors[4 * tx + vx + y * w].x = (colors[4 * tx + vx + y * w].x * 
                    currentSample + r.x) * (1.0 / (currentSample + 1.0));
            colors[4 * tx + vx + y * w].y = (colors[4 * tx + vx + y * w].y * 
                    currentSample + r.y) * (1.0 / (currentSample + 1.0));
            colors[4 * tx + vx + y * w].z = (colors[4 * tx + vx + y * w].z * 
                    currentSample + r.z) * (1.0 / (currentSample + 1.0));
        }
        seeds[(y + yOffset) * (2 * width) + 2 * (4 * tx + vx + xOffset)] = 
                seed0;
        seeds[(y + yOffset) * (2 * width) + 2 * (4 * tx + vx + xOffset) + 1] = 
                seed1;
    }
}
