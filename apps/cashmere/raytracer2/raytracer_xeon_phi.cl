...
__kernel void radianceKernel (int h, int w, __global Vec* colors, int height, 
    ...
    const int vx = get_local_id(0);
    const int tx = get_group_id(0);
    const int y = get_group_id(1);
    


    const int x = 16 * tx + vx;
    const int realY = y + yOffset;
    const int realX = 16 * tx + vx + xOffset;
    if (y < h) {
    
        uint seed0 = seeds[(y + yOffset) * (2 * width) + 2 * (16 * tx + vx + 
                xOffset)];
        uint seed1 = seeds[(y + yOffset) * (2 * width) + 2 * (16 * tx + vx + 
                xOffset) + 1];
        Ray ray;
        generateCameraRay(camera, &seed0, &seed1, width, height, 16 * tx + vx 
                + xOffset, y + yOffset, &ray);
        Vec r;
        radiancePathTracing(nrSpheres, spheres, &ray, &seed0, &seed1, &r);
        if (currentSample == 0) {
        
            vassign2m(&colors[16 * tx + vx + y * w], &r);
        } else {
        
            const float k1 = currentSample;
            const float k2 = 1.0 / (currentSample + 1.0);
            colors[16 * tx + vx + y * w].x = (colors[16 * tx + vx + y * w].x * 
                    currentSample + r.x) * (1.0 / (currentSample + 1.0));
            colors[16 * tx + vx + y * w].y = (colors[16 * tx + vx + y * w].y * 
                    currentSample + r.y) * (1.0 / (currentSample + 1.0));
            colors[16 * tx + vx + y * w].z = (colors[16 * tx + vx + y * w].z * 
                    currentSample + r.z) * (1.0 / (currentSample + 1.0));
        }
        seeds[(y + yOffset) * (2 * width) + 2 * (16 * tx + vx + xOffset)] = 
                seed0;
        seeds[(y + yOffset) * (2 * width) + 2 * (16 * tx + vx + xOffset) + 1] 
                = seed1;
    }
}
