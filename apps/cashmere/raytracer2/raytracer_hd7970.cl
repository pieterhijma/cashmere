...
__kernel void radianceKernel (int h, int w, __global Vec* colors, int height, 
    ...
    const int wtx = get_local_id(0);
    const int wtx0 = get_local_id(1);
    const int bx = get_group_id(0);
    const int y = get_group_id(1);
    


    const int nrWorkitemsNrThreadsW = 64;
    const int nrWavefrontsNrThreadsW = 4;
    const int tx = 64 * wtx0 + wtx;
    const int x = 256 * bx + (64 * wtx0 + wtx);
    const int realY = y + yOffset;
    const int realX = 256 * bx + (64 * wtx0 + wtx) + xOffset;
    if (y < h) {
    
        uint seed0 = seeds[(y + yOffset) * (2 * width) + 2 * (256 * bx + (64 * 
                wtx0 + wtx) + xOffset)];
        uint seed1 = seeds[(y + yOffset) * (2 * width) + 2 * (256 * bx + (64 * 
                wtx0 + wtx) + xOffset) + 1];
        Ray ray;
        generateCameraRay(camera, &seed0, &seed1, width, height, 256 * bx + 
                (64 * wtx0 + wtx) + xOffset, y + yOffset, &ray);
        Vec r;
        radiancePathTracing(nrSpheres, spheres, &ray, &seed0, &seed1, &r);
        if (currentSample == 0) {
        
            vassign2m(&colors[256 * bx + (64 * wtx0 + wtx) + y * w], &r);
        } else {
        
            const float k1 = currentSample;
            const float k2 = 1.0 / (currentSample + 1.0);
            colors[256 * bx + (64 * wtx0 + wtx) + y * w].x = (colors[256 * bx 
                    + (64 * wtx0 + wtx) + y * w].x * currentSample + r.x) * 
                    (1.0 / (currentSample + 1.0));
            colors[256 * bx + (64 * wtx0 + wtx) + y * w].y = (colors[256 * bx 
                    + (64 * wtx0 + wtx) + y * w].y * currentSample + r.y) * 
                    (1.0 / (currentSample + 1.0));
            colors[256 * bx + (64 * wtx0 + wtx) + y * w].z = (colors[256 * bx 
                    + (64 * wtx0 + wtx) + y * w].z * currentSample + r.z) * 
                    (1.0 / (currentSample + 1.0));
        }
        seeds[(y + yOffset) * (2 * width) + 2 * (256 * bx + (64 * wtx0 + wtx) 
                + xOffset)] = seed0;
        seeds[(y + yOffset) * (2 * width) + 2 * (256 * bx + (64 * wtx0 + wtx) 
                + xOffset) + 1] = seed1;
    }
